...
with app.app_context():
    ...
    if room:
        ...
        # Create a room_data dict with the fix applied
        room_data = {
            'id': room.id,
            'title': room.title,
        ...
        print("\nVerified: Room data now includes avg_amenity_rating:")
        print(json.dumps(room_data, indent=2))
        
        # Verify the value is correct
        print(f"\nVerifying value: {room_data['avg_amenity_rating']} matches database value {room.avg_amenity_rating}")
    else:
